...
import "./LoginPage.css";
import { setAuth, isAuthed } from "../auth"; // ✅ ใช้มาตรฐานเดียวกัน
...
export default function LoginPage() {
  ...
  // ✅ ถ้า login อยู่แล้ว ให้เด้งออกจากหน้า /login ด้วย useEffect (ไม่กระทบ UI)
  useEffect(() => {
    if (isAuthed()) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      // หน่วงนิดหน่อยเพื่อแสดงสถานะปุ่ม (ไม่จำเป็นก็ลบได้)
      await new Promise((r) => setTimeout(r, 300));

      // ตัวอย่างล็อกอินแบบ mock (ปรับเป็นเรียก API จริงได้เลย)
      let role = null;
      if (username === "user" && password === "user123") role = "USER";
      ...
      // ✅ เก็บตามมาตรฐานเดียวกัน (ต้องมี token + role เป็น 'USER'|'ADMIN')
      setAuth({
        token: "dummy-token",     // ถ้าเรียก API จริง ให้ใส่ token จริง
        role,
        user: { name: username },
      });

      // ✅ เด้งไปหน้าที่ควรอยู่: ถ้าโดนกันไว้ เด้งกลับที่มา, ไม่งั้นไปตาม role
